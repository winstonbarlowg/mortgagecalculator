...
print(f"At a LTV of {ltv}%, " + f"your minimum deposit would be of: = £{min_deposit}")
...
while deposit < min_deposit: 
    ...
    deposit = float(input(f'Enter a value that is equal or higher to £{min_deposit} '))
    if deposit > min_deposit:
        ...
        print(f"And the deposit as a percentage of the value would be of: = {deposit_percentage}%")
        break
...
mortgage_type = int(input('Enter mortgage type in years, e.g. 15 for 15 years: '))
...
print(f"You have a total of {loan_term} monthly repayments" + f" which add up to a total value of the loan of £{value_of_loan}")
